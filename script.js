document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateGuessBox(length, letter){
        guessBox[length].textContent = letter;
    }

    //Updates the guess as a whole by pushing the letter of the key pressed. 
    function updateGuessWord(arr, letter){
        arr.push(letter);
        return arr;
    }
    ...
    //Updates the guess as a whole by pushing the letter of the key pressed. 
    function shortenGuess(arr){
        arr.pop();
        return arr;
    }

    //makes a guess by comparing word (complicated to avoid bad runtime)
    function getColors(word, correctWord){
        let colors = [0, 0, 0, 0, 0];
        const dict = {};
        for (let i = 0; i < 5; i++){
            if(!(correctWord[i] in dict) ){
                dict[correctWord[i]] = correctWord[i];
            }
        }
        for (let i = 0; i < 5; i++){
    ...
    function colorCaller(ind, element){
        if(ind == 0){
            turnGray(element);
        }
        else if(ind == 1){
    ...
    function guesser(word, correctWord){
        ...
        for(let i = 0; i < 5; i++){
            colorCaller(colors[i], keyList[i]);
        }
